...
export default {
 // ============ 隐写 / 图像 ============
  ...
  zeroWidth: {
    ...
    usage: "把可疑文本（复制粘贴一段看似普通的话）粘进来解码，取出零宽字符携带的隐藏信息；也可编码藏字。字符集可选预设（默认档 / zwsp3 三字符 / zwspFull5 五字符——对应 zwsp-steg-js 的 ZWSP/FULL 模式口径）或自定义；二进制模式按字节藏文件（拖入文件走 rawBytes，解出非 UTF-8 自动给 .bin 下载）。",
    examples: [
    ...
    tips: [
      ...
      "他工具互操作：ZWSP 3 字符档 / FULL 5 字符档分别对齐 offdev zwsp-steg-js 的两种模式（已双向互操作实测）；二进制档对齐 Kei Misawa misc_tools 的字节口径。",
    ],
