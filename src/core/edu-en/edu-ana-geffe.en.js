...
export default {
  geffe: {
    what: "Geffe generator — An LFSR combining generator proposed by P. R. Geffe in 1973: three independent LFSRs are combined by the nonlinear function $f(x1,x2,x3) = x1 \\cdot x2 \\oplus x2 \\cdot x3 \\oplus x3$ to produce a keystream. Once thought \"nonlinear = secure\", but Siegenthaler (1984) showed f has 3/4 correlation with x1 and x3, enabling a correlation attack that recovers each LFSR by brute force — a high-frequency CTF crypto stream cipher topic.",
    principle:
      ...
      "Complexity comparison: brute-forcing Geffe directly = `2^(L1+L2+L3)`; correlation attack = `2^L1 + 2^L3 + 2^L2` (L2 portion) — an exponential reduction. E.g. with L1=L2=L3=20, direct brute force $2^{60}$ is infeasible, but correlation attack $2^{20}+2^{20}+2^{20} \\approx 3\\mathrm{M}$ runs in seconds.",
    usage:
      ...
      "Attack mode: paste the keystream (0/1 string, tolerates spaces/newlines/commas) into the input box, fill in the 3 tap parameters, optionally check bruteL2 (works when L2 ≤ 22). When L > 22 brute force is infeasible ($2^{22} \\approx 4\\mathrm{M}$); use `lfsrRecover` (Berlekamp-Massey) instead.",
    examples: [
      {
        in: "",
    ...
    tips: [
      ...
      "Longer keystream = more accurate recovery: distinguishing 0.75 from 0.5 statistically needs enough samples, recommend $N \\ge 10 \\times \\max(L1, L3)$. N=30 may fail (short-sample noise), N≥40 is usually stable.",
