...
export default {
	...
	grostl: {
		...
		tips: [
			...
			"输出变换再做一次 $P(h) \\oplus h$（截尾输出），是 Grøstl 防长度扩展的特征设计。",
		],
	...
	pearson: {
		what: "Pearson 哈希——Peter K. Pearson 1990 年在 CACM 发表的极简快速非加密哈希。逐字节查 256 排列表 $T[h \\oplus c]$ 累积，输出 1~32 字节。",
		principle:
			"Pearson 哈希极其简洁：维护一个 8 位寄存器 h，初始为 0。对消息每个字节 c：\n\n" +
			"$$h = T[h \\oplus c]$$\n\n" +
			"其中 T 是一张 0..255 的随机置换表（每个值恰好出现一次）。处理完所有字节后 h 即是单字节哈希。\n\n" +
			"多字节输出：用「首字节替身扩展」——每多生成一个字节，就对 h 异或 (i-1)（第 i 字节）再查 T 得到新字节。即 $h_1 = T[h_0 \\oplus 0]$、$h_2 = T[h_0 \\oplus 1]$、$h_3 = T[h_0 \\oplus 2]$…，所有 $h_i$ 拼成输出。\n\n" +
	...
	yenc: {
		...
		principle:
			"yEnc 的编码规则极为简洁：\n\n" +
			"1. 每个字节 $+42 \\bmod 256$，得到编码后的字节 c。\n" +
			"2. 转义：若 c 是 NUL(0x00)、CR(0x0d)、LF(0x0a)、'='（0x3d）、TAB（0x09，行首）、空格（0x20，行首）、'.'（0x2e，行首），则在该字节前加 '='（转义标志），并将该字节 $+64 \\bmod 256$。\n" +
	...
	skipjack: {
		...
		principle:
			...
			"核心是 key 相关置换 G_k（4 轮 Feistel）：每轮用 1 个 key 字节（按 $4k \\bmod 10$ 轮转）查 256 项 F 表再异或。\n\n" +
			"解密用 G 的逆置换 h，规则反走，同一把 key 的 tab 预处理（tab[i][c]=$F[c \\oplus \\mathrm{key}[9-i]]$）。\n\n" +
	...
	threefish: {
		...
		principle:
			...
			"子密钥（第 s 个子密钥）：前 Nw-3 个字 = 循环取密钥字 $K[(s+j) \\bmod (\\mathrm{Nw}+1)]$，后 3 个字分别加上 $T[s \\bmod 3]$、$T[(s+1) \\bmod 3]$、s（轮计数）。其中 $K_{\\mathrm{Nw}} = C240 \\oplus K0 \\oplus \\cdots \\oplus K_{\\mathrm{Nw}-1}$（C240 是固定魔数），$T2 = T0 \\oplus T1$。\n\n" +
