...
export default {
  geffe: {
    what: "Geffe 生成器——P. R. Geffe 1973 提出的 LFSR 组合生成器：3 个独立 LFSR 的输出经非线性函数 $f(x1,x2,x3) = x1 \\cdot x2 \\oplus x2 \\cdot x3 \\oplus x3$ 组合成 keystream，曾被认为「非线性=安全」，但 Siegenthaler 1984 指出 f 与 x1/x3 有 3/4 相关性，可用相关攻击逐 LFSR 穷举恢复——CTF crypto 流密码高频考点。",
    principle:
      ...
      "复杂度对比：直接穷举 Geffe = `2^(L1+L2+L3)`；相关攻击 = `2^L1 + 2^L3 + 2^L2`（L2 部分）——指数级降低。如 L1=L2=L3=20，直接穷举 $2^{60}$ 不可行，相关攻击 $2^{20}+2^{20}+2^{20} \\approx 3\\mathrm{M}$ 秒级。",
    usage:
      ...
      "attack 模式：把 keystream（0/1 串，容忍空格/换行/逗号）粘进输入框，3 个抽头参数照填，可选勾 bruteL2（L2 ≤ 22 时穷举+验证）。L>22 时穷举不可行（$2^{22} \\approx 4\\mathrm{M}$），改用 `lfsrRecover`（Berlekamp-Massey）辅助。",
    examples: [
      {
        in: "",
    ...
    tips: [
      ...
      "keystream 越长越准：统计区分 0.75 vs 0.5 需要足够样本，建议 $N \\ge 10 \\times \\max(L1, L3)$。N=30 可能恢复错（短样本噪声），N≥40 通常稳定。",
      "LFSR2 P=0.5 无法用相关攻击直接恢复——必须勾 bruteL2 穷举+L1/L3 验证，或对 keystream 直接跑 lfsrRecover（Berlekamp-Massey）求等效 LFSR。",
      "级数 L>22 时 $2^{L}$ 穷举不可行（$2^{22} \\approx 4\\mathrm{M}$），需用代数攻击（Grobner 基 / BM + 已知明文）——本工具会显式提示「L 过大」。",
      "组合函数 $f = x1 x2 \\oplus x2 x3 \\oplus x3$ 等价于「x2 是选择器」：$x2=0$ 输出 x3，$x2=1$ 输出 x1。看到这种结构立刻想到 Geffe。",
