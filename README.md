...
【未收录算法大补 26 件】按 dCode 全站工具清单交叉对照补齐 26 个算法，覆盖古典密码、编码映射、压缩校验、工程编码四类，全部过权威对拍。现共 753 个算法。

【拼音首字母搜索】ksmm → 凯撒密码：6763 字全拼音表索引，全拼/首字母双命中。

【Malbolge 执行】解释器+规范形互转，官方 7 样例 17/17 逐字节。

【文本盲水印 / QR 格式爆破】guofei 格式文本盲水印（原版互通 20/20）；QR 格式信息 32 组合爆破。

【18 语言统一】20 语言全与英文基线同构（1471 键）；四门 RTL 语言零隐形控制字符。

### 修复 BUG

【科普卡公式】68 分片 201 处补渲染定界；行内解析先取代码段——`$7z$…` 哈希串不再误入公式。
...
- **753 个注册操作 · 18 分类**：749 个纯前端操作 + 4 个可选 Windows 本地桥操作，覆盖 Base、文本传输、花式 CTF、中文本土、古典密码、现代加密、哈希校验、进制字符集、分析爆破、密码攻击、取证文件、数据结构、隐写图像、深奥语言及本地桥分类。完整清单由 registry 自动生成。
